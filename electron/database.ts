...
export default class PumpDatabase {
  private db: Database.Database

  constructor() {
    const dbPath = join(app.getPath('userData'), 'pumping_station.db')
  ...
  seed() {
    ...
    pumps.forEach((pump, idx) => {
      const result = insertPump.run(
        pump.name, pump.code, pump.location, pump.longitude, pump.latitude, pump.basin,
        pump.designFlow, pump.head, pump.power, pump.equipmentModel, pump.manufacturer,
        pump.installDate, pump.ratedCurrent, pump.maxCurrent, pump.forebayWarningLevel,
        pump.forebayDangerLevel, JSON.stringify(pump.upstreamPumpIds), JSON.stringify(pump.downstreamPumpIds),
        'standby', dayjs().subtract(1, 'month').format('YYYY-MM-DD'),
        dayjs().add(2, 'month').format('YYYY-MM-DD')
      )

      const pumpId = result.lastInsertRowid as number
    ...
    for (let i = 0; i < 7; i++) {
      ...
      insertWeather.run(
        date, 25 + Math.random() * 8, 60 + Math.random() * 30, rainfall,
        rainfall > 30 ? (rainfall > 50 ? 'storm' : 'heavy') : (rainfall > 10 ? 'moderate' : 'light'),
        rainfall * 1.2, 2 + Math.random() * 5,
    ...
    pipelineNodes.forEach((node) => {
      insertNode.run(
        node.code, node.name, node.type, node.longitude, node.latitude,
        JSON.stringify(node.connected), 1.5 + Math.random() * 1.0, node.maxLevel, node.warningLevel
      )
    })
  ...
  createPump(data: any): number {
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `).run(
      data.name, data.code, data.location, data.longitude, data.latitude, data.basin,
      data.designFlow, data.head, data.power, data.equipmentModel, data.manufacturer,
      data.installDate, data.ratedCurrent, data.maxCurrent, data.forebayWarningLevel,
      data.forebayDangerLevel, JSON.stringify(data.upstreamPumpIds || []),
      JSON.stringify(data.downstreamPumpIds || []), data.status || 'standby',
      data.lastMaintenanceDate, data.nextMaintenanceDate
    )
    return result.lastInsertRowid as number
  }

  updatePump(id: number, data: any): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      data.installDate, data.ratedCurrent, data.maxCurrent, data.forebayWarningLevel,
      data.forebayDangerLevel, JSON.stringify(data.upstreamPumpIds || []),
      JSON.stringify(data.downstreamPumpIds || []), data.status, id
    )
  }
  ...
  getSchedules(): any[] {
    return this.db.prepare('SELECT * FROM schedules ORDER BY scheduleDate DESC, id DESC').all().map((s: any) => ({
      ...s,
  ...
  getSchedule(id: number): any {
    const schedule = this.db.prepare('SELECT * FROM schedules WHERE id = ?').get(id) as any
  ...
  createSchedule(data: any): number {
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      data.scheduleDate, data.pumpId, data.pumpName, data.planStartTime, data.planEndTime,
      data.plannedFlow, JSON.stringify(data.operatingUnits || []), data.reason, data.constraints,
      data.status || 'pending_approval'
    )
    return result.lastInsertRowid as number
  }

  updateSchedule(id: number, data: any): void {
    this.db.prepare(`
  ...
  approveSchedule(id: number, approver: string, comment: string): void {
    this.db.prepare(`
  ...
  requestScheduleAdjust(id: number, operator: string, reason: string): void {
    this.db.prepare(`
  ...
  getRealtimeMonitoring(pumpId: number): any {
    return this.db.prepare(`
      SELECT * FROM monitoring_data WHERE pumpId = ? ORDER BY timestamp DESC LIMIT 1
    `).get(pumpId)
  }

  getMonitoringHistory(pumpId: number, start: string, end: string): any[] {
    return this.db.prepare(`
  ...
  insertMonitoring(data: any): number {
    let isAlert = 0
    ...
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      data.pumpId, data.unitId, data.current, data.voltage || 380, data.flow, data.head,
      data.forebayLevel, data.outletPressure || 0, data.vibration || 0, data.temperature || 25,
      isAlert, alertLevel
    )
    return result.lastInsertRowid as number
  }
  ...
  createMaintenanceOrder(data: any): number {
    ...
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      orderNo, data.pumpId, data.unitId, data.type, data.priority, data.description,
      data.triggerReason, JSON.stringify(data.triggerValue || {}), data.assignedTeam,
      JSON.stringify(data.partsRequired || []), data.planDate, data.remark
    )
    return result.lastInsertRowid as number
  }

  updateMaintenanceOrder(id: number, data: any): void {
    this.db.prepare(`
      ...
      WHERE id = ?
    `).run(
      data.type, data.priority, data.description, data.assignedTeam, data.status,
      JSON.stringify(data.partsRequired || []), data.planDate, data.actualStartDate,
      data.actualEndDate, data.remark, id
    )
  }

  generateMaintenanceOrders(): number[] {
  ...
  updateInventory(id: number, data: any): void {
    this.db.prepare(`
  ...
  deductInventory(partId: number, quantity: number, operator: string = '系统', reason: string = '维保工单扣减', orderNo?: string): boolean {
    const item = this.db.prepare('SELECT * FROM inventory WHERE id = ?').get(partId) as any
  ...
  inboundInventory(partId: number, quantity: number, operator: string, reason: string): boolean {
    const item = this.db.prepare('SELECT * FROM inventory WHERE id = ?').get(partId) as any
  ...
  outboundInventory(partId: number, quantity: number, operator: string, reason: string): boolean {
    return this.deductInventory(partId, quantity, operator, reason)
  }

  getInventoryRecords(partId?: number): any[] {
    if (partId) {
      return this.db.prepare(`
  ...
  getStatisticsSummary(start: string, end: string): any {
    const result = this.db.prepare(`
      SELECT
  ...
  getPumpStatistics(pumpId: number, start: string, end: string): any {
    ...
    `).get(pumpId, start, end) as any).count
  ...
  getBasinStatistics(start: string, end: string): any[] {
    const pumps = this.getPumps()
  ...
  updateWeatherForecast(data: any[]): void {
    ...
    const tx = this.db.transaction((items: any[]) => {
      items.forEach(item => {
        stmt.run(item.forecastDate, item.temperature, item.humidity, item.rainfall,
          item.rainfallIntensity, item.rainfall24h, item.windSpeed, item.tideLevel)
      })
    })
  ...
  getPipelineNetworkLevels(): any[] {
    return this.db.prepare('SELECT * FROM pipeline_nodes ORDER BY id').all().map((n: any) => ({
      ...n,
